...
export default function App() {
  ...
  // ------------------ Auto-detect JWT on page reload ------------------
  useEffect(() => {
    if (token) {
      try {
  ...
  const submitContact = async () => {
    try {
      ...
      setMessage(err.response?.data?.message || "Contact error");
    }
  };
  ...
  const fetchAdminData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API}/admin-dashboard`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setAdminData({ contacts: res.data.contacts, resumes: res.data.resumes });
    } catch (err) {
      setMessage(err.response?.data?.message || "Admin fetch error");
    }
  };

  const downloadResume = async (filename) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API}/download-resume/${filename}`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: "blob"
      });
      ...
      {role === "admin" && (
        <section className="mt-6">
          ...
          <div className="mb-4">
            ...
            {adminData.contacts.length === 0 ? <p>No messages yet</p> : (
              <table className="w-full text-black rounded overflow-hidden">
                ...
                <tbody>
                  {adminData.contacts.map((c, i) => (
                    <tr key={i} className="bg-gray-100 even:bg-gray-200">
                      <td className="p-2">{c.name}</td><td className="p-2">{c.email}</td><td className="p-2">{c.message}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            ...
            {adminData.resumes.length === 0 ? <p>No resumes uploaded</p> : (
              <table className="w-full text-black rounded overflow-hidden">
                <thead className="bg-gray-300">
                  <tr><th className="p-2">User</th><th className="p-2">Filename</th></tr>
                </thead>
                <tbody>
                  {adminData.resumes.map((r, i) => (
                    <tr key={i} className="bg-gray-100 even:bg-gray-200">
                      <td className="p-2">{r.user}</td>
                      <td className="p-2 flex justify-between items-center">
                        {r.filename}
                        <button
                          className="ml-4 bg-green-600 px-2 py-1 rounded hover:bg-green-700"
                          onClick={() => downloadResume(r.filename)}
                        >
                          Download
                        </button>
                      </td>
